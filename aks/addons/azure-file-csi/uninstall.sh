...
[[ "$FORCE" != "true" && "$DRY_RUN" != "true" ]] && { read -rp "Disable Azure File CSI? (yes/no): " c; [[ "$c" != "yes" ]] && exit 0; }
run_cmd() { [[ "$DRY_RUN" == "true" ]] && echo "[DRY RUN] $*" || eval "$@"; }
run_cmd "az aks update --name $CLUSTER --resource-group $RESOURCE_GROUP --disable-file-driver"
echo "Azure File CSI driver disabled!"
