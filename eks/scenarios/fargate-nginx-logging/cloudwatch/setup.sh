#!/usr/bin/env bash
# Fargate Nginx Logging to CloudWatch Scenario
...
Setup Fargate logging to CloudWatch.
...
main() {
    ...
    if [[ -z "$role_name" ]]; then
        log_error "No Fargate profile found"
        exit 1
    fi
    ...
    log_step "Complete"
    log_info "Fargate logging to CloudWatch configured"
    log_info "Next: Apply ConfigMap for Fluent Bit configuration"
    exit 0
}
