#!/usr/bin/env bash
# NVIDIA GPU Operator installation for Kubernetes
...
OPTIONS:
    ...
    --namespace NS        Namespace (default: gpu-operator)
    --mig-strategy STR    MIG strategy: none, single, mixed (default: mixed)
    --driver-enabled      Enable GPU driver installation (default: true)
    --no-driver           Disable GPU driver (use host driver)
...
EXAMPLES:
    ...
    # Install with host driver
    $(basename "$0") --no-driver

    # Install specific version
    $(basename "$0") --chart-version 24.6.0
...
uninstall() {
    ...
    if helm list -n "$NAMESPACE" | grep -q "$RELEASE_NAME"; then
        if $DRY_RUN; then
            ...
            log_success "Uninstalled GPU Operator"
        fi
    else
...
main() {
    ...
    if $DRY_RUN; then
        ...
        log_info "Would install GPU Operator"
        log_info "  Chart Version: $CHART_VERSION"
        log_info "  Namespace: $NAMESPACE"
        log_info "  Driver Enabled: $DRIVER_ENABLED"
        log_info "  MIG Strategy: $MIG_STRATEGY"
        exit 0
    fi

    # Install/Upgrade
    log_step "Installing GPU Operator"
    ...
    log_info "Check GPU nodes: kubectl get nodes -l nvidia.com/gpu.present=true"
    exit 0
}
