#!/usr/bin/env bash
set -euo pipefail
NS="karpenter"; RN="karpenter"; FORCE=false; DELETE_CRDS=false; DRY_RUN=false
...
[[ "$FORCE" != "true" && "$DRY_RUN" != "true" ]] && { read -rp "Uninstall Karpenter? (yes/no): " c; [[ "$c" != "yes" ]] && exit 0; }
run_cmd "helm uninstall $RN -n $NS" || true
[[ "$DELETE_CRDS" == "true" ]] && run_cmd "kubectl delete crd nodepools.karpenter.sh ec2nodeclasses.karpenter.k8s.aws nodeclaims.karpenter.sh" || true
