#!/usr/bin/env bash
set -euo pipefail
NS="karpenter"; TO_VERSION=""; DRY_RUN=false
...
helm status karpenter -n "$NS" &>/dev/null || { echo "Not installed"; exit 1; }
cmd="helm upgrade karpenter oci://public.ecr.aws/karpenter/karpenter -n $NS"
[[ -n "$TO_VERSION" ]] && cmd="$cmd --version $TO_VERSION"
run_cmd "$cmd"
echo "Karpenter upgraded!"
