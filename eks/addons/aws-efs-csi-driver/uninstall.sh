...
[[ "$FORCE" != "true" && "$DRY_RUN" != "true" ]] && { read -rp "Uninstall EFS CSI Driver? (yes/no): " c; [[ "$c" != "yes" ]] && exit 0; }
run_cmd "aws eks delete-addon --cluster-name $CLUSTER_NAME --addon-name aws-efs-csi-driver" || true
echo "AWS EFS CSI Driver uninstalled!"
