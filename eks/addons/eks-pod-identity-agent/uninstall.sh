...
[[ "$FORCE" != "true" && "$DRY_RUN" != "true" ]] && { read -rp "Uninstall Pod Identity Agent? (yes/no): " c; [[ "$c" != "yes" ]] && exit 0; }
run_cmd "aws eks delete-addon --cluster-name $CLUSTER_NAME --addon-name eks-pod-identity-agent" || true
echo "EKS Pod Identity Agent uninstalled!"
