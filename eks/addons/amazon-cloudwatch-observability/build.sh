#!/usr/bin/env bash
# Amazon CloudWatch Observability installation for EKS (via EKS Addon)
...
Install Amazon CloudWatch Observability on an EKS cluster as an EKS Addon.
...
EXAMPLES:
    ...
    $(basename "$0") --addon-version v1.5.0-eksbuild.1
...
# Uninstall
uninstall() {
    log_step "Uninstalling CloudWatch Observability"
...
main() {
    ...
    if $DRY_RUN; then
        ...
        log_info "Would install CloudWatch Observability"
    ...
    if ! $SKIP_IAM; then
        ...
        role_arn=$(eksctl get iamserviceaccount \
            ...
            --region "$REGION" \
            --output json | \
            jq -r ".[] | select(.metadata.namespace == \"$NAMESPACE\" and .metadata.name == \"$SERVICE_ACCOUNT_NAME\") | .status.roleARN")
        
        log_info "Role ARN: $role_arn"
    fi

    # Install/Update addon
    log_step "Installing CloudWatch Observability"
