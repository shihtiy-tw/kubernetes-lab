...
cmd="aws eks update-addon --cluster-name $CLUSTER_NAME --addon-name amazon-cloudwatch-observability --resolve-conflicts OVERWRITE"
[[ -n "$TO_VERSION" ]] && cmd="$cmd --addon-version $TO_VERSION"
run_cmd "$cmd"
echo "CloudWatch Observability upgraded!"
