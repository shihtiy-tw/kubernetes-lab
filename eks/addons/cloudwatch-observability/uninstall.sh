...
[[ "$FORCE" != "true" && "$DRY_RUN" != "true" ]] && { read -rp "Uninstall CloudWatch addon? (yes/no): " c; [[ "$c" != "yes" ]] && exit 0; }
run_cmd "aws eks delete-addon --cluster-name $CLUSTER_NAME --addon-name amazon-cloudwatch-observability" || true
echo "CloudWatch Observability uninstalled!"
