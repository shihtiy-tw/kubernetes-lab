#!/usr/bin/env bash
set -euo pipefail
NS="kube-system"; RN="cluster-autoscaler"; FORCE=false; DRY_RUN=false
...
[[ "$FORCE" != "true" && "$DRY_RUN" != "true" ]] && { read -rp "Uninstall Cluster Autoscaler? (yes/no): " c; [[ "$c" != "yes" ]] && exit 0; }
run_cmd "helm uninstall $RN -n $NS" || true
echo "Cluster Autoscaler uninstalled!"
