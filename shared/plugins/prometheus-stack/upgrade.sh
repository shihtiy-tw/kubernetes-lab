#!/usr/bin/env bash
set -euo pipefail
NS="monitoring"; RN="prometheus-stack"; TO_VERSION=""; DRY_RUN=false
...
cmd="helm upgrade $RN prometheus-community/kube-prometheus-stack -n $NS"
[[ -n "$TO_VERSION" ]] && cmd="$cmd --version $TO_VERSION"
run_cmd "$cmd"
echo "prometheus-stack upgraded!"
