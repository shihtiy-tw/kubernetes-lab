#!/usr/bin/env bash
set -euo pipefail
NAMESPACE="kube-system"; RELEASE_NAME="metrics-server"; FORCE=false; DRY_RUN=false
...
[[ "$FORCE" != "true" && "$DRY_RUN" != "true" ]] && { read -rp "Uninstall metrics-server? (yes/no): " c; [[ "$c" != "yes" ]] && exit 0; }
